...
def _patient_record(patient_id: str, entries: list[dict],
                    email: str = "p@example.com",
                    batch_date: str = "2026-05-03") -> dict:
    return {
...
def test_build_conditions_df_filters_to_condition_resources(spark):
    ...
    records = [_patient_record("MRN-1", [
        {"resource_type": "Condition", "code": "I10", "description": "Hyp",
         "category": "Circulatory", "is_chronic": True, "onset_date": "2024-01-01",
         "status": "active", "clinician_npi": "NPI-1"},
        {"resource_type": "MedicationStatement", "medication": "metformin",
         "generic_name": "Metformin HCl", "drug_class": "Biguanides",
         "dosage": "500mg", "frequency": "once_daily", "start_date": "2024-01-01",
         "end_date": None, "status": "active", "prescriber_npi": "NPI-2"},
    ])]
...
def test_build_medications_df_emits_scd2_columns(spark):
    ...
    records = [_patient_record("MRN-1", [
        {"resource_type": "MedicationStatement", "medication": "metformin",
         "generic_name": "Metformin HCl", "drug_class": "Biguanides",
         "dosage": "500mg", "frequency": "once_daily",
         "start_date": "2024-01-01", "end_date": None,
         "status": "active", "prescriber_npi": "NPI-2"},
    ])]
...
def test_build_medications_df_marks_stopped_meds_not_current(spark):
    ...
    records = [_patient_record("MRN-1", [
        {"resource_type": "MedicationStatement", "medication": "metformin",
         ...
         "status": "stopped", "prescriber_npi": "NPI-2"},
    ])]
...
def test_build_labs_df_computes_observation_id(spark):
    ...
    records = [_patient_record("MRN-1", [
        {"resource_type": "Observation", "code": "HbA1c", "value": 6.4,
         "unit": "%", "reference_low": 4.0, "reference_high": 5.6,
         "is_abnormal": True, "date": "2026-04-01"},
    ])]
