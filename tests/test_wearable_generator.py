...
from data_generators.synthetic.wearable_generator import (  # noqa: E402
    DEVICE_METRICS,
    USERS,
    generate_reading,
)
...
class TestWearableEventStructure:
    ...
    def test_different_metrics_per_device_type(self):
        ...
        assert "heart_rate_bpm" in sw_event["metrics"], "Smartwatch must have heart_rate_bpm"
        assert (
            "blood_glucose_mgdl" not in sw_event["metrics"]
        ), "Smartwatch should NOT have blood_glucose_mgdl"

        assert (
            "blood_glucose_mgdl" in gm_event["metrics"]
        ), "Glucose monitor must have blood_glucose_mgdl"
        assert (
            "heart_rate_bpm" not in gm_event["metrics"]
    ...
    def test_some_late_events(self):
        ...
        for _ in range(200):
            ...
            total += 1
            diff = (
                parse_ts(event["sync_timestamp"]) - parse_ts(event["event_timestamp"])
            ).total_seconds()
