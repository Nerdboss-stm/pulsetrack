# Dev environment — small instances, spot pricing, aggressive auto-terminate.
environment = "dev"

emr_release_label        = "emr-7.13.0"
emr_master_instance_type = "m5.xlarge"
emr_core_instance_type   = "m5.xlarge"
emr_core_instance_count  = 4 # bumped from 2 — 2-core starved YARN under 3 concurrent streaming queries (see ADR-008)
emr_core_spot_bid_price  = "" # on-demand for streaming workloads (was "0.08" spot)
emr_ebs_volume_size_gb   = 64
emr_idle_timeout_seconds = 7200 # 2h auto-terminate

budget_limit_usd = "40"
