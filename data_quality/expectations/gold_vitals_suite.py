...
def prepare_for_validation(gold_df: DataFrame) -> DataFrame:
    ...
    return gold_df.withColumn(
        ...
        F.when(
            ...
            (F.col("min_value") <= F.col("avg_value"))
...
def assert_no_orphan_patient_keys(gold_df: DataFrame, dim_patient_df: DataFrame) -> int:
    ...
    """
    return (
        gold_df.select("patient_key").distinct()
...
def build():
    ...
    suite.add_expectation(gxe.ExpectColumnValuesToBeInSet(
        column="agg_envelope_ok", value_set=[True],
    ))

    # pct_in_normal_range bounded to [0, 100]
    suite.add_expectation(gxe.ExpectColumnValuesToBeBetween(
        column="pct_in_normal_range", min_value=0, max_value=100,
    ))
